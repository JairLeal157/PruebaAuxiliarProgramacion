...
from schemas.Product import Product, example_product, existe_producto
...
async def get_products():
    ...
    if len(result) == 0:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No hay productos")
    return result
...
async def update_product(id: int, product: Product = Body(example=example_product)):
    ...
    with engine.connect() as connection:
        product.id = id
        dict_product = product.dict()
        connection.execute(productTable.update().values(
            dict_product).where(productTable.c.id == id))
    return dict_product
...
async def delete_product(id: int):
    ...
    with engine.connect() as connection:
        connection.execute(
            productTable.delete().where(productTable.c.id == id))
    return
