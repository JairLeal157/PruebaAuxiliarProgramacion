...
from schemas.User import User, example_user, encriptar, verificar, existe_user
...
async def get_users():
    ...
    with engine.connect() as connection:
        ...
        if len(result) == 0:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No hay usuarios")
    return result
...
async def get_user(id: int):
    with engine.connect() as connection:
        result = connection.execute(
            userTable.select().where(userTable.c.id == id)).fetchall()
        if len(result) == 0:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No existe el usuario")
        result = result[0]
    return result
...
async def update_user(id: int, user: User = Body(example=example_user)):
    ...
    with engine.connect() as connection:
        ...
        user.contraseña = encriptar(user.contraseña)
        dict_user = user.dict()
        connection.execute(userTable.update().values(
            dict_user).where(userTable.c.id == id))
        print()
    return dict_user
